...
describe("Comecon App", () => {
	...
	describe("Test Case 35", () => {
		...
		it('Verify that an ingredient is registered without "unidad"', () => {
			...
			cy.on("window:alert", (txt) => {
				expect(txt).to.include("la unidad debe tener solo caracteres de la a la z y no debe estar vacia");
			});
		});
		...
		it('Verify that an ingredient is registered without "ingrediente"', () => {
			...
			cy.on("window:alert", (txt) => {
				expect(txt).to.include("el nombre debe tener solo caracteres de la a la z y no debe estar vacia");
			});
		});
	});
